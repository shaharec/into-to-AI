...
def independentPair(a1, a2):
  ...
  """
  """
  Checkibg inconsistent effects:
  1. same action then true.
  2. a1 check:
    - a1 deleted precondition does not appear in a2 preconditions.
    - a1 deleted actions doesnt appear in a2 effect.
  3. a2 check the same as a1 check.
  """
  # Same action returns true.
  if a1 == a2:
    return True

  # a1 checking
  for del_con in a1.getDelete():
    if a2.isPreCond(del_con) or a2.isPosEffect(del_con):
      return False

  # a2 checking
  for del_con in a2.getDelete():
    if a1.isPreCond(del_con) or a1.isPosEffect(del_con):
      return False
